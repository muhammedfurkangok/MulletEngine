...
// Replaces file_manager extension in |in_file_name| with |new_extension|.
...
// Returns the file_manager extension in lowercase if present, else "". Extension is
// defined as the string after the last '.' character. If the file_manager starts with
...
// Returns the file_manager name without extension.
std::string RemoveFileExtension(const std::string &filename);

// Given a path of the input file_manager |input_file_relative_path| relative to the
// parent directory of |sibling_file_full_path|, this function returns full path
// to the input file_manager. If |sibling_file_full_path| has no directory, the relative
// path itself |input_file_relative_path| is returned. A common use case is for
// the |input_file_relative_path| to be just a file_manager name. See usage examples in
// the unit test.
...
// of file_manager referenced by |file_name| into |buffer| and returns true upon
// success.
...
// Convenience method for reading a file_manager into a std::string. Reads contents
// of file_manager referenced by |file_name| into |contents| and returns true upon
// success.
...
// of |buffer| to file_manager referred to by |file_name|. File is overwritten if it
...
// file_manager referenced by |file_name|. Returns 0 when referenced file_manager is empty or
// does not exist.
