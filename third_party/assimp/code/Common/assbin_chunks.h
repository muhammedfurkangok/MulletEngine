...
to serialize 3D-models and to reload them quickly. Assimp's file_manager formats are designed to
...
(<tt>.assbin</tt>) is fast loading. Optional deflate compression helps reduce file_manager size. The XML
...
@section assbin Binary file_manager format

The ASSBIN file_manager format is composed of chunks to represent the hierarchical aiScene data structure.
...
for use by stand-alone ASSBIN loaders. Also, Assimp's own file_manager writer can be found
...
integer     Major version of the Assimp library which wrote the file_manager
integer     Minor version of the Assimp library which wrote the file_manager
...
short       0 for normal files, 1 for shortened dumps for regression tests
                these should have the file_manager extension assbin.regress
...
byte[256]   Zero-terminated source file_manager name, UTF-8
...
// these are the magic chunk identifiers for the binary ASS file_manager format
